...
# Cache size (configurable via environment)
CACHE_SIZE = int(os.environ.get("LOOP_EMBEDDING_CACHE", "1000"))
...
def similarity(text1: str, text2: str) -> float:
    ...
    Falls back to simple sequence matching if model unavailable.
    """
    emb1 = embed(text1)
    emb2 = embed(text2)
...
def cosine_similarity_blob(blob1: bytes, blob2: bytes) -> float:
    ...
    emb1 = blob_to_embed(blob1)
    emb2 = blob_to_embed(blob2)
    return cosine_similarity(emb1, emb2)
